...
var GiacGeneric = P(MathOutput, function(_, super_) {
  ...
  _.parseSavedProperties = function(args) {
	...
	_.postInsertHandler = function() {
		this.varStoreField = registerFocusable(MathQuill, this, 'var_store', { ghost: 'ans', handlers: {
...
var createGiacElement = function(options) {
	...
	var html = '<table class="' + css_prefix + 'giac_element"><tbody><tr><td class="' + css_prefix + 'var_store">'
 	+ '<div class="' + css_prefix + 'focusableItems" data-id="0"><span class="' + css_prefix + 'var_store">' + focusableHTML('MathQuill',  'var_store') + '<span class="equality">&#8801;</span></span>' + focusableHTML('CodeBlock',  options.name) + '</td>'
	...
	giac_elements_to_add.push({key: options.name, el: P(GiacGeneric, function(_, super_) {
		...
		_.postInsertHandler = function() {
			...
			this.focusableItems[0].unshift(registerFocusable(CodeBlock, this, this.options.name, {}));
			super_.postInsertHandler.call(this);
			return this;
		}
