...
var fusion = P(Element, function(_, super_) {
	...
	_.postInsertHandler = function() {
		...
		window.ajaxRequest("/fusion/get_vars", {id: this.worksheet.server_id}, function(_this) { return function(response) { 
			...
			_this.jQ.find('div.sync').html('<a href="fusion360://command=open&file=na&privateInfo=SwiftCalcs_load_' + response.id + '" onclick="$(this).addClass(\'grey\');return true;" target="_blank" class="button grey">Sync to Fusion 360</a> <span class="explain">Make sure Fusion 360 is open on your machine</span>')
