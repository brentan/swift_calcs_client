	...
	_.evaluationFinished = function(result, evaluation_id) {
		...
		if(this.lastTrueId != new_true_id) {
			...
			if(new_true_id > -1) {
	    		...
	    		el.commandChildren(function(_this) { if(_this.jQ) { _this.jQ.removeClass(css_prefix + "greyout"); } if(_this.evaluatable) { _this.altered_content = true;  _this.previous_commands = []; } });
	    }
