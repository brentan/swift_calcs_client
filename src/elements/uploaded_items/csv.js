...
var csvBlock = P(uploadedItem, function(_, super_) {
	_.validVarName = false;
	_.headerRow = false;
	_.colList = false;
	_.maxCols = 0;
	_.scoped = true;
	...
	_.data = "";
	_.helpText = "<[VAR]> = <<import csv>> FILE\nLoad the CSV data from FILE into the provided variable name VAR.  Use the preview data table to select the columns you wish to import by checking/unchecking the boxes above each column.  If there is a header row, select the row using the radio button to the left of the row.  When a header row is selected, data will be placed into a variable with the name VAR and subscript COLUMN, such as VAR<sub>COLUMN</sub>.  When no header row is selected, data is loaded as a 2D matrix into VAR.";

	_.innerHtml = function() {
		return '<div class="' + css_prefix + 'top ' + css_prefix + 'focusableItems" data-id="0">' + focusableHTML('MathQuill',  'var_name') + '<span class="equality">&#8801;</span>' + focusableHTML('CodeBlock', 'import csv') + '&nbsp;<span class="file_name"></span>' + helpBlock() + '<BR>' + answerSpan() + '</div><div class="' + css_prefix + 'insert"></div>';
	}
	_.postInsertHandler = function() {
		...
		this.insertJQ.append('<div style="text-align:center;"><span class="fa fa-spinner fa-pulse fa-fw"></span>Loading CSV Data</div>');
		return this;
	}
	...
	_.selectData = function() {
		var results = this.data;
		this.jQ.removeClass('error');
		this.insertJQ.html('');
		$('<div style="text-align:center;"><strong>Select Columns to Import</strong></div>').appendTo(this.insertJQ);
		var $div = $('<div class="table"></div>');
		var $table = $('<table/>');
		var $tbody = $('<tbody/>');
		max_cols = 0;
		for(var i = 0; i < min(30,results.data.length); i++) 
			max_cols = max(max_cols, results.data[i].length);
		this.maxCols = max_cols;
		var next = '<td class="header" style="border-right-width: 1px;font-size:0.8em;">Header Row?</td>';
		for(var i = 0; i < max_cols; i++) 
			next += '<td class="header"><i class="fa fa-fw fa-check-square-o" data-col="' + i + '"></i></td>';
		$tbody.append('<tr>' + next + '</tr>');
		for(var i = 0; i < min(30,results.data.length); i++) {
			next = '<td class="header"><i class="fa fa-fw fa-circle-o" data-row="' + i + '"></i></td>';
			for(var j = 0; j < results.data[i].length; j++) 
				next += '<td>' + results.data[i][j] + '</td>';
			for(j; j < max_cols; j++)
				next += '<td></td>';
			$tbody.append('<tr>' + next + '</tr>');
		}
		...
		$table.appendTo($div);
		var _this = this;
		$table.on('click', '.fa-check-square-o', function(e) {
			$(this).removeClass('fa-check-square-o').addClass('fa-square-o');
			var col = $(this).attr('data-col')*1+1;
			var c = 0;
			$table.find('tr').each(function() {
				c++;
				if(c <= 1) return;
				$(this).find('td').eq(col).addClass('col_grey');
			});
			e.preventDefault();
			e.stopPropagation();
		});
		$table.on('click', '.fa-square-o', function(e) {
			...
			$table.find('tr').each(function() {
				c++;
				if(c <= 1) return;
				$(this).find('td').eq(col).removeClass('col_grey');
			});
			e.preventDefault();
			e.stopPropagation();
		});
		$table.on('click', '.fa-circle-o', function(e) {
			$table.find('.fa-dot-circle-o').removeClass('fa-dot-circle-o').addClass('fa-circle-o');
			$table.find('.row_grey').removeClass('row_grey');
			$table.find('.row_strong').removeClass('row_strong');
			$(this).addClass('fa-dot-circle-o').removeClass('fa-circle-o');
			var row = $(this).attr('data-row')*1+1;
			_this.headerRow = row-1;
			var r = $table.find('tr').eq(row);
			r.find('td').addClass('row_strong');
			for(var c = 1; c < row; c++)
				$table.find('tr').eq(c).find('td').addClass('row_grey');
			_this.submissionHandler(_this)(_this.varName);
			e.preventDefault();
			e.stopPropagation();
		});
		$table.on('click', '.fa-dot-circle-o', function(e) {
			_this.headerRow = false;
			$table.find('.fa-dot-circle-o').removeClass('fa-dot-circle-o').addClass('fa-circle-o');
			$table.find('.row_grey').removeClass('row_grey');
			$table.find('.row_strong').removeClass('row_strong');
			_this.submissionHandler(_this)(_this.varName);
		...
		$sub_link.click(function(e) { 
			var c = 0;
			_this.colList = new Array(max_cols);
			$table.find('tr').eq(0).find('td').each(function() {
				c++;
				if(c <= 1) return;
				_this.colList[c-2] = $(this).find('i.fa').hasClass('fa-check-square-o');
			});
			_this.colList = _this.colList.join('_');
			_this.loadData();
			_this.submissionHandler(_this)(_this.varName);
			e.preventDefault();
			e.stopPropagation();
		...
		if(this.colList) {
			...
			$table.find('tr').eq(0).find('td').each(function() {
				c++;
				if(c <= 1) return;
				if(_this.colList[c-2] == 'false') $(this).find('i.fa').click();
			});
		}
		this.colList = false;
	...
	_.loadData = function() {
		...
		$div.appendTo(this.insertJQ);
		// Load the data into a string for giac to process
		var cols = this.colList.split('_');
		this.commands = [];
		for(var i = 0; i < cols.length; i++)
			cols[i] = (cols[i] == 'true');
		if(this.headerRow === false) {
			// Store all data into a single 2D matrix
			var rows = [];
			...
			this.commands.push({sub: '', data: '[[' + rows.join('],[') + ']]'});
		} else {
			// Use the header row to determine names
			for(var j = 0; j < this.maxCols; j++) {
				if(!cols[j]) continue;
				var rows = [];
				for(var i = (this.headerRow+1); i < results.data.length; i++) {
					if(j >= results.data[i].length)
						var val = 0;
					else
						var val = (typeof results.data[i][j] == "number" ? results.data[i][j] : 0)
	...
	_.setURL = function(url) {
		...
		Papa.parse(this.url, {
			...
			error: function(err) {
				_this.jQ.addClass('error');
				_this.insertJQ.html('');
				var $span = $('<div class="error"></div>');
				$span.html('Error while loading CSV File: ' + err.message + '<BR>');
				$span.append('<a href="#">Try Again</a>');
				$span.find('a').on('click', function(e) {
					csv().insertBefore(_this).show();
					_this.remove(0);
				});
				_this.insertJQ.append($span);
			}
		});
		return this;
	}
  _.toString = function() {
  	return '{csvBlock}{{' + this.argumentList().join('}{') + '}}';
  }
