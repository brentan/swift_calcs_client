/*
Library will take textual representations of mathematical expressions
such as "c = x^2 + 2/5" and convert into mathquill syntax.

Based on MathJS parser by Jos de Jong
*/
...
function factory (type, config, load, typed) {
  ...
  function getToken() {
    ...
    // skip comment - BRENTAN: For matlab conversion, do we start intersplicing text and math blocks?
    if (c == '#') {
      while (c != '\n' && c != '') {
        next();
      }
  ...
  function parseFunctionAssignment () {
    ...
    if (token_type == TOKENTYPE.SYMBOL && token == 'function') {
      // BRENTAN: Transform into the corrected form?
